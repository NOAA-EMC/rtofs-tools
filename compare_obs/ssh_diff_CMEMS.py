...
from grids import create_grid_CMEMS_SSH, create_grid_RTOFS, create_regular_grid
from plotting import plot_glo_ssh_obs_bias_3rows
from utils import str2bool, valid_date

warnings.filterwarnings("ignore", module="xesmf")

def read_CMEMS_ssh(args, var="adt"):
    """Read Sea Surface Height (SSH) from CMEMS observations.
    ...
    var : str, optional
        The variable name to extract from the CMEMS file. Default is "ssh".

    Returns
    -------
    xarray.DataArray
        The 2D squeezed and offset-adjusted CMEMS SSH data array.
    """
    ...
    obsfile_thisdate = glob(f"{args.obs}/dt_global_allsat_phy_l4_{cdate}_*.nc")
    print(obsfile_thisdate)
    # there should be only one file per date
    assert len(obsfile_thisdate) == 1
    ncfile = obsfile_thisdate[0]
    print(f"reading {var} from {ncfile}")
    ssh = xr.open_dataset(ncfile)[var].squeeze()
    return ssh


def read_HYCOM_ssh(args, var="ssh"):
    """Read Sea Surface Height (SSH) from HYCOM RTOFS output.
    ...
    var : str, optional
        The variable name to extract from the HYCOM file. Default is "ssh".

    Returns
    -------
    xarray.DataArray
        The 2D HYCOM SSH data array (with the extra boundary row removed).
    """
    ...
    if (days_offset > 0.0) or args.use_forecast_t0:  # is forecast
        ...
        ncfile = (
            f"{args.ref_model}/rtofs.{cdate}/rtofs_glo_2ds_f{tplus:03g}_diag.nc"
        )
    else:
        if days_offset < -1:
        ...
        ncfile = (
            f"{args.ref_model}/rtofs.{cdate}/rtofs_glo_2ds_n{tminus:03g}_diag.nc"
        )
    print(f"reading {var} from {ncfile}")
    ssh = xr.open_dataset(ncfile)[var].squeeze()[:-1, :]  # hycom has one more row
    return ssh


def read_MOM_ssh(args, var="ssh"):
    """Read Sea Surface Height (SSH) from MOM RTOFS output.
    ...
    args : argparse.Namespace
        ...
        ('days_offset').
    var : str, optional
        The variable name to extract from the MOM file. Default is "ssh".

    Returns
    -------
    xarray.DataArray
        The 2D MOM SSH data array.
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Script to plot RTOFS SSS against CMEMS."
    )
    ...
    ssh_cmems = read_CMEMS_ssh(args)
    if args.ref_model_is_hycom:
        ssh_ref = read_HYCOM_ssh(args)
    ssh_dev = read_MOM_ssh(args)
    ...
    assert len(obsfile_thisdate) == 1
    ncfile = obsfile_thisdate[0]
    grid_cmems = create_grid_CMEMS_SSH(ncfile)
    print("CMEMS grid created")

    grid_rtofs_ref = create_grid_RTOFS(args.grid, ssh_ref)
    print("RTOFS grid created")
    grid_rtofs_dev = create_grid_RTOFS(args.grid, ssh_dev)
    print("RTOFS grid created")
    ...
    if os.path.exists("esmf_wgts_cmems_ssh_1x1.nc"):
        ...
        print("remapping CMEMS, this may takes some time...")
        remap_cmems = xesmf.Regridder(
            grid_cmems, grid_1x1, "conservative_normed", periodic=True
        )
        remap_cmems.to_netcdf("esmf_wgts_cmems_ssh_1x1.nc")

    cmems_1x1deg = remap_cmems(ssh_cmems)
    # compute global sum for rescaling
    glossh_cmems = cmems_1x1deg.weighted(grid_1x1["area"]*grid_1x1["mask"]).mean(dim=("x", "y"))
    frac = grid_1x1["area"]/(grid_1x1["area"]*grid_1x1["mask"]).sum(dim=("x", "y"))
    ...
    rtofs_ref_1x1 = remap_rtofs_ref(ssh_ref)
    # compute global sum for rescaling
    glossh_rtofs_ref = rtofs_ref_1x1.weighted(grid_1x1["area"]*grid_1x1["mask"]).mean(dim=("x", "y"))
    scale_rtofs_ref = glossh_cmems / glossh_rtofs_ref
    ...
    rtofs_dev_1x1 = remap_rtofs_dev(ssh_dev)
    # compute global sum for rescaling
    glossh_rtofs_dev = rtofs_dev_1x1.weighted(grid_1x1["area"]*grid_1x1["mask"]).mean(dim=("x", "y"))
    scale_rtofs_dev = glossh_cmems / glossh_rtofs_dev

    #import matplotlib.pyplot as plt
    #plt.figure()
    #plt.pcolormesh(cmems_1x1deg -glossh_cmems, vmin=-2, vmax=2, cmap="jet")
    #plt.figure()
    #plt.pcolormesh(rtofs_ref_1x1 -glossh_rtofs_ref, vmin=-2, vmax=2, cmap="jet")
    #plt.figure()
    #plt.pcolormesh(rtofs_dev_1x1 -glossh_rtofs_dev, vmin=-2, vmax=2, cmap="jet")
    #plt.show()

    fig = plot_glo_ssh_obs_bias_3rows(
        grid_1x1,
        cmems_1x1deg -glossh_cmems,
        rtofs_ref_1x1 -glossh_rtofs_ref,
        rtofs_dev_1x1 -glossh_rtofs_dev,
        date=args.date,
    ...
    if args.days_offset < 0:
        fout = (
            f"compare_SSH_CMEMS_{args.date}_tminus{-1 * args.days_offset:02g}days.png"
        )
    else:
        fout = f"compare_SSH_CMEMS_{args.date}_tplus{args.days_offset:02g}days.png"
...
# example:
#python ssh_diff_CMEMS.py --ref_model /scratch5/NCEPDEV/rstprod/Santha.Akella/data/rtofs/v2p5/output --dev_model /scratch5/NCEPDEV/rstprod/Santha.Akella/data/rtofs/v3_exp/May2026 --obs /scratch3/NCEPDEV/marine/Raphael.Dussin/obs/gridded/SSH_CMEMS/my --date 2025-12-21 --ref_model_is_hycom yes --grid /scratch3/NCEPDEV/global/role.glopara/fix/mom6/20250128/008/ocean_hgrid.nc --days_offset 0
