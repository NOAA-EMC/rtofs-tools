...
module load libjpeg/${libjpeg_ver}
module load grib_util/${grib_util_ver}
module load ve/hafs/${vehafs_ver}
module list
...
export job=$jobname
mkdir -p ${myDATAROOT}/$jobid
cat << EOF_momincup > $batchloc/rtofs.incup.$pid
#!/bin/bash
#PBS -N $jobname
#PBS -j oe
#PBS -A $account
...
cat << EOF_momanal > $batchloc/rtofs.anal.$pid
#!/bin/bash
#PBS -N $jobname
#PBS -j oe
#PBS -A $account
...
export job=$jobname
mkdir -p ${myDATAROOT}/$jobid
cat << EOF_momfcst1 > $batchloc/rtofs.fcst1.$pid
#!/bin/bash
#PBS -N $jobname
#PBS -j oe
#PBS -A $account
...
export job=$jobname
mkdir -p ${myDATAROOT}/$jobid
cat << EOF_mom0 > $batchloc/rtofs.mom.$pid
#!/bin/bash
#PBS -N $jobname
#PBS -j oe
#PBS -A $account
