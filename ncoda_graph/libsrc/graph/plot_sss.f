      subroutine plot_sss (dir_path, date, dtg, tau, nest, m, n, depth,
     *                     mask, igrid, rlat, stdlt1, stdlt2, stdlon,
     *                     bl, br, tl, tr, i1, i2, j1, j2, sss_cnt, 
     *                     sss_del, sss_min, do_sss, do_err, do_inc, 
...
c      @(#)$Id$
c
c MODULE NAME:  plot_sss
c
c DESCRIPTION:  maps a CODA sss analysis and color fills the contours
c
c PARAMETERS:
...
c
     ...
     *                         .true., fail)
            write (title, '(''Sea Surface Salinity (PSU)'')')
            ...
            dmn = sss_min
            dmx = dmn + sss_del * real (n_slc)
            do_lbl = .true.
            lntr = 4
            nslc = n_slc
         else if (k .eq. 2 .and. tau .eq. 0 .and. do_inc) then
c
c           ..sss analyzed increment
c
            file_typ = 'analinc'
            call rd_coda_file (dir_path, dtg, nest, m, n, l, file_typ,
     *                         fld_name, fluid, tau_hr, lvl_typ, wrk,
     *                         .true., fail)
            write (title, '(''SSS Analyzed Increment (PSU)'')')
...
c
c           ..sss prediction error
c
            file_typ = 'fcsterr'
            call rd_coda_file (dir_path, dtg, nest, m, n, l, file_typ,
     *                         fld_name, fluid, tau_hr, lvl_typ, wrk, 
     *                         .true., fail)
            write (title, '(''SSS Prediction Error (PSU)'')')
...
c
     ...
     *                         .true., fail)
            do i = 1, (m * n)
               wrk(i) = wrk(i) * 100.
            enddo
            write (title, '(''SSS Analysis Error Reduction (%)'')')
            call gks_color (rgb_err_clr, MX_ERR_CLR)
            cntr_int = 20.
            do_cntr = .true.
            do_lbl = .false.
