      subroutine plot_xsect_ensm (dir_path, ensm_path, date, dtg, m,
     *                            n, l, n_mem, igrid, nest, delx, dely,
     *                            iref, jref, reflat, reflon, stdlt1,
     *                            stdlt2, stdlon, n_xsect, slat, slon,
     *                            elat, elon, edpth, zlvl, lyr, spval)
c
c.............................START PROLOGUE............................
c
...
c      @(#)$Id$
c
c MODULE NAME:  plot_xsect_ensm
c
c DESCRIPTION:  plots cross sections of ensemble perturbations
c
c PARAMETERS:
...
c
c     ..retrieve lat, lon, and dummy lvl arrays
c
      call rd_ll_lvl (dir_path, dtg, lyr, m, n, l, nest, lat,
     *                lon, grd)
c
...
c
      do kv = 1, nv
         ...
         else if (kv .eq. 2) then
            fld_name = 'salint'
            dmn = -0.015
            dmx =  0.015
            cntr_int = 0.1
            title1 = 'Salinity (PSU)'
         else if (kv .eq. 3) then
            fld_name = 'uucurr'
            dmn = -0.15
            dmx =  0.15
            cntr_int = 0.1
            title1 = 'U Velocity (cm/s)'
         else if (kv .eq. 4) then
            fld_name = 'vvcurr'
            dmn = -0.15
            dmx =  0.15
            cntr_int = 0.1
            title1 = 'V Velocity (cm/s)'
         else
            cycle
         endif
c
c        ..loop over ensemble members
c
         do nm = 1, n_mem
c
c           ..retrieve ensemble perturbation grid
c
            write (sfx, '(a, ''_inc_mem'', i3.3, ''_nest'', i2.2,
     *                    ''.'', a)') fld_name, nm, nest, dtg
            file_name = trim (ensm_path) // '/' // trim (sfx)
...
c
c           ..convert m/s to cm/s
c
           if (kv .eq. 3 .or. kv. eq. 4) then
              do i = 1, (m * n * l)
                 if (grd(i) .gt. spmis) grd(i) = grd(i) * 100.
              enddo
           endif
c
...
c
               call smth_2d (n_pass, xsect_data, MX_HORIZ, MX_VERT,
     *                       spval)
c
...
c
               ...
               write (title2, '(''Ensemble Perturbations'',
     *                          '' - Member '', i2)') nm
               len = len_trim (title2)
...
c
      ...
      deallocate (iag, iamap, lat, lon, mask, obs_xi, obs_yj, obs_zk)
      deallocate (pdepth, pdist, plat, plon, pxi, pyj, pzk, xcs)
      deallocate (xsect_data, xsect_mask, ycs)
c
      return
      end
