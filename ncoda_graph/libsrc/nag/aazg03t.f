      SUBROUTINE AAZG03(MATRIX,WEIGHT,N,X,LDX,M,ISX,IVAR,WT,T,V,LDV,S,E)
...
C     .. Array Arguments ..
      REAL              E(IVAR), S(*), V(LDV,*), WT(*), X(LDX,M)
      INTEGER           ISX(M)
C     .. Local Scalars ..
      REAL              SCALE, SUM, XBAR
      INTEGER           I, J, K
...
C     .. Executable Statements ..
   ...
   20          CONTINUE
            ...
            ELSE
               DO 40 I = 1, N
                  IF (WT(I).GT.0.0) SUM = SUM + WT(I)*X(I,J)
   40          CONTINUE
               E(K) = SUM/T
            END IF
         END IF
         ...
         IF (MATRIX.EQ.'C' .OR. MATRIX.EQ.'c') THEN
            DO 300 J = 1, M
               IF (ISX(J).GT.0) THEN
                  ...
                  SUM = 0.0
                  DO 260 I = 1, N
                     IF (WT(I).GT.0.0) SUM = SUM + WT(I)*((X(I,J)-XBAR)
     *                                       **2)
  260             CONTINUE
                  IF (SUM.GT.0.0) THEN
                     SUM = SQRT(SUM)
