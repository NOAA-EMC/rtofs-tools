      SUBROUTINE F01QDE(TRANS,WHERET,M,N,A,LDA,ZETA,NCOLB,B,LDB,WORK,
     *                  IFAIL)
...
C     =======
C
C  F01QDE performs one of the transformations
C
C     B := Q*B   or   B := Q'*B,
C
...
C     .. External Subroutines ..
      EXTERNAL          SGEMV, SGER, ABWP01, ABYP01
...
C
     ...
     *    ) CALL ABWP01(TRANS,'TRANS',IFAIL,IERR,SRNAME)
     ...
     *    IERR,SRNAME)
      ...
      IF (LDB.LT.M) CALL ABYP01(LDB,'LDB',IFAIL,IERR,SRNAME)
      IF (IERR.GT.0) THEN
...
C
            CALL SGEMV('Transpose',M-K+1,NCOLB,ONE,B(K,1),LB,A(K,K),1,
     *                 ZERO,WORK,1)
C
C           Now form  C := C - u*work'.
C
            CALL SGER(M-K+1,NCOLB,-ONE,A(K,K),1,WORK,1,B(K,1),LB)
C
