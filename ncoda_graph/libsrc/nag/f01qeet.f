      SUBROUTINE F01QEE(WHERET,M,N,NCOLQ,A,LDA,ZETA,WORK,IFAIL)
...
C     =======
C
C  F01QEE  returns  the  first  ncolq columns  of the  m by m orthogonal
...
C     .. External Subroutines ..
      EXTERNAL          F06FBE, F06QHE, ABWP01, ABYP01, SGEMV, SGER,
     *                  SSCAL
...
C
     ...
     *    IERR,SRNAME)
      ...
      IF ((NCOLQ.LT.0) .OR. (NCOLQ.GT.M)) CALL ABYP01(NCOLQ,'NCOLQ',
     *    IFAIL,IERR,SRNAME)
...
C
      ...
      END IF
      P = MIN(N,NCOLQ)
      IF (P.GT.1) CALL F06QHE('Upper',P-1,P-1,ZERO,ZERO,A(1,2),LDA)
      IF (NCOLQ.GT.N) THEN
         NCQ = NCOLQ - N
C
...
C        matrix.
C
         CALL F06QHE('General',N,NCOLQ-N,ZERO,ZERO,A(1,N+1),LDA)
         CALL F06QHE('General',M-N,NCOLQ-N,ZERO,ONE,A(N+1,N+1),LDA)
      ELSE
         NCQ = 0
      END IF
      DO 20 K = P, 1, -1
...
C
            IF ((K.LT.M) .AND. (NCQ.GT.0)) THEN
               CALL SGEMV('Transpose',M-K+1,NCQ,ONE,A(K,K+1),LDA,A(K,K),
     *                    1,ZERO,WORK,1)
C
C              Now form  C := C - u*work'.
C
               CALL SGER(M-K+1,NCQ,-ONE,A(K,K),1,WORK,1,A(K,K+1),LDA)
            END IF
C
C           Now form the kth column of Q.
C
            CALL SSCAL(M-K+1,-ZETAK,A(K,K),1)
            A(K,K) = ONE + A(K,K)
         ELSE
            A(K,K) = ONE
            IF (K.LT.M) CALL F06FBE(M-K,ZERO,A(K+1,K),1)
         END IF
         NCQ = NCQ + 1
   20 CONTINUE
C
