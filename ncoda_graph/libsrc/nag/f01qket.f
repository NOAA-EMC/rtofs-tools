      SUBROUTINE F01QKE(WHERET,M,N,NROWP,A,LDA,ZETA,WORK,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C
C  1. Purpose
C     =======
C
C  F01QKE  returns the first  nrowp rows of the n by n orthogonal matrix
...
C     .. Parameters ..
      ...
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F01QKE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, M, N, NROWP
      CHARACTER*1       WHERET
C     .. Array Arguments ..
      REAL              A(LDA,*), WORK(*), ZETA(*)
C     .. Local Scalars ..
      REAL              ZETAK
...
C     .. External Subroutines ..
      EXTERNAL          SGEMV, SGER, SSCAL, F06FBE, F06QHE, ABWP01,
     *                  ABYP01
...
C
     ...
     *     .AND. (WHERET.NE.'s')) CALL ABWP01(WHERET,'WHERET',IFAIL,
     *    IERR,SRNAME)
      IF (M.LT.0) CALL ABYP01(M,'M',IFAIL,IERR,SRNAME)
      IF (N.LT.M) CALL ABYP01(N,'N',IFAIL,IERR,SRNAME)
      IF ((NROWP.LT.0) .OR. (NROWP.GT.N)) CALL ABYP01(NROWP,'NROWP',
     *    IFAIL,IERR,SRNAME)
      IF (LDA.LT.MAX(M,NROWP)) CALL ABYP01(LDA,'LDA',IFAIL,IERR,SRNAME)
      IF (IERR.GT.0) THEN
...
C     to zero.
C
      IF (M.GT.1) CALL F06QHE('Upper',M-1,M-1,ZERO,ZERO,A(1,2),LDA)
      IF (NROWP.GT.M) THEN
         NRP = NROWP - M
C
...
C        matrix.
C
         CALL F06QHE('General',NRP,M,ZERO,ZERO,A(MP1,1),LDA)
         CALL F06QHE('General',NRP,N-M,ZERO,ONE,A(MP1,MP1),LDA)
      ELSE
         NRP = 0
      END IF
      DO 20 K = 1, M
...
C
            CALL SGEMV('No transpose',K-1,K-1,ONE,A,LDA,A(K,1),LDA,ZERO,
     *                 WORK,1)
            CALL SGEMV('No transpose',K-1,N-M,ONE,A(1,MP1),LDA,A(K,MP1),
     *                 LDA,ONE,WORK,1)
C
C           Now  form  all  but  the   last  row   of  the  new   B1  as
C           B1 := B1 - v1*u1'.
C
            CALL SGER(K-1,K,-ONE,WORK,1,A(K,1),LDA,A,LDA)
C
C           Form  all   but   the   last  row   of   the   new   B2   as
C           B2 := B2 - v1*u2'.
C
            CALL SGER(K-1,N-M,-ONE,WORK,1,A(K,MP1),LDA,A(1,MP1),LDA)
C
C           Then form   v2 = B4*u2 + B3*u1,  and store it in  work( 1 ),
C           ..., work( nrp ).
C
            CALL SGEMV('No transpose',NRP,N-M,ONE,A(MP1,MP1),LDA,
     *                 A(K,MP1),LDA,ZERO,WORK(K),1)
            CALL SGEMV('No transpose',NRP,K-1,ONE,A(MP1,1),LDA,A(K,1),
     *                 LDA,ONE,WORK(K),1)
C
C           Form the new  B3  as  B3 := B3 - v2*u1'.
C
            CALL SGER(NRP,K,-ONE,WORK(K),1,A(K,1),LDA,A(MP1,1),LDA)
C
C           Form the new  B4  as  B4 := B4 - v2*u2'.
C
            CALL SGER(NRP,N-M,-ONE,WORK(K),1,A(K,MP1),LDA,A(MP1,MP1),
     *                LDA)
C
C           Now form the last rows of the new  B1 and B2.
C
            CALL SSCAL(K,-ZETAK,A(K,1),LDA)
            A(K,K) = ONE + A(K,K)
            CALL SSCAL(N-M,-ZETAK,A(K,MP1),LDA)
         ELSE
            A(K,K) = ONE
            CALL F06FBE(K-1,ZERO,A(K,1),LDA)
            CALL F06FBE(N-M,ZERO,A(K,MP1),LDA)
         END IF
   20 CONTINUE
C
      IFAIL = P01ABE(IFAIL,0,SRNAME,0,REC)
