      SUBROUTINE F02SXE(N,A,LDA,NCOLY,Y,LDY,WORK,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C
C  1. Purpose
C     =======
C
C  F02SXE returns either the matrix Z given by
C
C     Z = P'*Y,
C
...
C
C  This routine must be preceded by a call to routine F02SWE.
C
C  2. Description
C     ===========
C
C  Routine  F02SWE factorizes the  n by n upper triangular matrix  R  as
C
C     R = Q*B*P',
C
...
C  upper triangular part of the array A. Following a call to F02SWE this
...
C     .. Parameters ..
      ...
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F02SXE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, LDY, N, NCOLY
C     .. Array Arguments ..
      REAL              A(LDA,*), WORK(*), Y(LDY,*)
C     .. Local Scalars ..
      INTEGER           IERR, J, K
C     .. Local Arrays ..
      CHARACTER*80      REC(1)
...
C     .. External Subroutines ..
      EXTERNAL          F06BCE, F06FBE, F06QXE, ABYP01
...
C
            DO 20 J = K + 2, N
               CALL F06BCE(A(K,J),WORK(J-1),WORK(N+J-2))
   20       CONTINUE
C
C           Form P( k )*Y.
C
            CALL F06QXE('Left','Variable pivot','Backwards',N,NCOLY,K+1,
     *                  N,WORK,WORK(N),Y,LDY)
   40    CONTINUE
      ELSE
C
            ...
            IF (N.GT.2) THEN
               DO 80 K = N - 2, 1, -1
                  ...
                  DO 60 J = K + 2, N
                     ...
                     A(K,J) = ZERO
   60             CONTINUE
                  CALL F06FBE(N-K,ZERO,A(K+1,K),1)
                  CALL F06QXE('Right','Variable pivot','Forward',N-K,
