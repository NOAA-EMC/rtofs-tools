      SUBROUTINE F02WEE(M,N,A,LDA,NCOLB,B,LDB,WANTQ,Q,LDQ,SV,WANTP,PT,
     *                  LDPT,WORK,IFAIL)
...
C     =======
C
C  F02WEE  returns all, or part, of the  singular value decomposition of
...
C
C           The array  WORK  is used as  internal  workspace by  F02WEE.
...
C  a call to the INTEGER function F06KLE.  The statement:
C
C     IRANK = F06KLE( MIN( M, N ), SV, 1, TOL )
C
...
C  relative machine precision ( see routine X02AJE ) is used in place of
C  TOL.
C
C
...
C     .. Parameters ..
      ...
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F02WEE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, LDB, LDPT, LDQ, M, N, NCOLB
      LOGICAL           WANTP, WANTQ
C     .. Array Arguments ..
      REAL              A(LDA,*), B(LDB,*), PT(LDPT,*), Q(LDQ,*), SV(*),
     *                  WORK(*)
C     .. Local Scalars ..
...
C     .. External Subroutines ..
      EXTERNAL          F01QEE, F01QCE, F01QDE, F01QJE, F01QKE, F02WUE,
     *                  F06QFE, ABYP01, SCOPY, SGEMV
...
C
      ...
      END IF
      IERR = 0
      IF (M.LT.0) CALL ABYP01(M,'M',IFAIL,IERR,SRNAME)
      IF (N.LT.0) CALL ABYP01(N,'N',IFAIL,IERR,SRNAME)
      IF (LDA.LT.M) CALL ABYP01(LDA,'LDA',IFAIL,IERR,SRNAME)
      IF (NCOLB.LT.0) CALL ABYP01(NCOLB,'NCOLB',IFAIL,IERR,SRNAME)
      IF ((LDB.LT.M) .AND. (NCOLB.GT.0)) CALL ABYP01(LDB,'LDB',IFAIL,
     *    IERR,SRNAME)
      IF ((LDQ.LT.M) .AND. (M.LT.N) .AND. (WANTQ)) CALL ABYP01(LDQ,
     *    'LDQ',IFAIL,IERR,SRNAME)
      IF ((LDPT.LT.N) .AND. (M.GE.N) .AND. (WANTQ) .AND. (WANTP))
     *    CALL ABYP01(LDPT,'LDPT',IFAIL,IERR,SRNAME)
      IF (IERR.GT.0) THEN
...
C     start of  theta  ( see routine F01QCE ) and when  m.lt.n  then  k3
C     marks the start of theta ( see routine F01QJE ). When WORK is used
C     to hold a matrix, the matrix always starts at WORK( 1 ).  k3 marks
C     the start of the workspace in the calls to  F02WUE.
C
      IF (M.GE.N) THEN
         IF (WANTQ) THEN
            IF (WANTP) THEN
...
C
         CALL F01QCE(M,N,A,LDA,WORK(K1),IER)
         IF (NCOLB.GT.0) CALL F01QDE('Transpose','Separate',M,N,A,LDA,
...
C              R = Q2*D*P'.
C
               CALL F06QFE('Upper triangular',N,N,A,LDA,PT,LDPT)
               CALL F01QEE('Separate',M,N,N,A,LDA,WORK(K1),WORK(K2),IER)
               CALL F02WUE(N,PT,LDPT,NCOLB,B,LDB,.TRUE.,WORK,N,SV,
     *                     .TRUE.,WORK(K3),IERR)
            ELSE
C
...
C
               CALL F02WUE(N,A,LDA,NCOLB,B,LDB,.TRUE.,WORK,N,SV,.FALSE.,
     *                     WORK(K3),IERR)
               CALL F01QEE('Separate',M,N,N,A,LDA,WORK(K1),WORK(K4),IER)
            END IF
C
C           Form  Q = Q1*Q2,  row by row using  Q' = Q2'*Q1'.
C
            DO 20 I = 1, M
               CALL SCOPY(N,A(I,1),LDA,WORK(K4),1)
               CALL SGEMV('Transpose',N,N,ONE,WORK,N,WORK(K4),1,ZERO,
     *                    A(I,1),LDA)
   20       CONTINUE
         ELSE
C
C           Find the SVD of R.
C
            CALL F02WUE(N,A,LDA,NCOLB,B,LDB,.FALSE.,Q,LDQ,SV,WANTP,WORK,
     *                  IERR)
         END IF
      ELSE
C
...
C
         CALL F01QJE(M,N,A,LDA,WORK(K3),IER)
         IF (WANTP) THEN
C
...
C           R = Q*D*P2'.
C
            CALL F06QFE('Upper triangular',M,M,A,LDA,WORK,M)
            CALL F01QKE('Separate',M,N,M,A,LDA,WORK(K3),WORK(K4),IER)
            CALL F02WUE(M,WORK,M,NCOLB,B,LDB,WANTQ,Q,LDQ,SV,.TRUE.,
     *                  WORK(K3),IERR)
C
C           Form  P' = P2'*P1'.
C
            DO 40 J = 1, N
               CALL SCOPY(M,A(1,J),1,WORK(K4),1)
               CALL SGEMV('No transpose',M,M,ONE,WORK,M,WORK(K4),1,ZERO,
     *                    A(1,J),1)
   40       CONTINUE
         ELSE
C
C           Find the SVD of R.
C
            CALL F02WUE(M,A,LDA,NCOLB,B,LDB,WANTQ,Q,LDQ,SV,.FALSE.,WORK,
     *                  IERR)
         END IF
      END IF
      IF (K3.GT.1) CALL SCOPY(MIN(M,N),WORK(K3),1,WORK,1)
      IF (IERR.NE.0) THEN
         WRITE (REC,FMT=99998) IERR
         IFAIL = P01ABE(IFAIL,IERR,SRNAME,2,REC)
      ELSE
         IFAIL = P01ABE(IFAIL,0,SRNAME,0,REC)
      END IF
      RETURN
