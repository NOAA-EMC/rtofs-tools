      SUBROUTINE F02WUE(N,A,LDA,NCOLB,B,LDB,WANTQ,Q,LDQ,SV,WANTP,WORK,
     *                  IFAIL)
...
C     =======
C
C  F02WUE  returns all, or part, of the  singular value decomposition of
...
C
C           The array   WORK  is used as  internal workspace by  F06XUE.
...
C     .. Parameters ..
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F02WUE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, LDB, LDQ, N, NCOLB
      LOGICAL           WANTP, WANTQ
C     .. Array Arguments ..
      REAL              A(LDA,*), B(LDB,*), Q(LDQ,*), SV(*), WORK(*)
C     .. Local Scalars ..
...
C     .. External Subroutines ..
      EXTERNAL          F02SWE, F02SXE, F02SYE, ABYP01
...
C
      ...
      IF ((LDB.LT.N) .AND. (NCOLB.GT.0)) CALL ABYP01(LDB,'LDB',IFAIL,
     *    IERR,SRNAME)
      IF ((LDQ.LT.N) .AND. (WANTQ)) CALL ABYP01(LDQ,'LDQ',IFAIL,IERR,
     *    SRNAME)
      IF (IERR.GT.0) THEN
...
C
      CALL F02SWE(N,A,LDA,SV,WORK,NCOLB,B,LDB,WANTQ,Q,LDQ,IERR)
      IF (WANTP) THEN
         CALL F02SXE(N,A,LDA,0,DUMMY,1,WORK(N),IERR)
         NCOLP = N
      ELSE
         NCOLP = 0
      END IF
...
C
      IERR = 1
      CALL F02SYE(N,SV,WORK,NCOLB,B,LDB,NCOLQ,Q,LDQ,NCOLP,A,LDA,WORK(N),
     *            IERR)
      IF (IERR.NE.0) THEN
