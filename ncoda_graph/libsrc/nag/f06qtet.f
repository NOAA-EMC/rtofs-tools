      SUBROUTINE F06QTE(SIDE,N,K1,K2,C,S,A,LDA)
...
C     ..
C
C  F06QTE performs the transformation
C
...
C     .. Local Scalars ..
      REAL              AIJ, CTEMP, FILL, STEMP, TEMP
      INTEGER           I, I1, J
...
C           columns.
C
            CALL F06BAE(A(J+1,J+1),FILL,CTEMP,STEMP)
            C(J) = CTEMP
            S(J) = -STEMP
...
C              element.
C
               CALL F06BAE(A(J,J),FILL,C(J),S(J))
            END IF
   60    CONTINUE
C
