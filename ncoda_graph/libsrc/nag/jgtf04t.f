      SUBROUTINE JGTF04(N,X,SCALE,SUMSQ,TINY,UNDFLW)
C     MARK 8 RELEASE. NAG COPYRIGHT 1979.
C     MARK 11.5(F77) REVISED. (SEPT 1985.)
C     MARK 13 REVISED. USE OF MARK 12 X02 FUNCTIONS (APR 1988).
C     WRITTEN BY S. HAMMARLING, MIDDLESEX POLYTECHNIC (SCLSQS)
C
C     JGTF04 RETURNS THE VALUES SCL AND SUM SUCH THAT
C
...
C     TINY = SQRT(X02AME) ,
C
C     WHERE X02AME IS THE SMALL VALUE RETURNED FROM ROUTINE X02AME.
C
C     UNDFLW MUST BE THE VALUE RETURNED BY X02DAE
C
C     .. Scalar Arguments ..
      REAL              SCALE, SUMSQ, TINY
      INTEGER           N
      LOGICAL           UNDFLW
C     .. Array Arguments ..
      REAL              X(N)
...
C
      DO 40 I = 1, N
         IF (X(I).EQ.0.0) GO TO 40
C
         ABSXI = ABS(X(I))
         IF (SCALE.GE.ABSXI) GO TO 20
C
         SUMSQ = 1.0 + SUMSQ*(SCALE/ABSXI)**2
         SCALE = ABSXI
         GO TO 40
C
...
C
   80    IF (ABSXI.GT.TINY) Q = ABSXI*TINY
         IF (SCALE.GE.Q) SUMSQ = 1.0 + SUMSQ*(SCALE/ABSXI)**2
         SCALE = ABSXI
C
  100 CONTINUE
C
