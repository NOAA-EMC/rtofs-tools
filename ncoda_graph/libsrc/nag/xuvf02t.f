      SUBROUTINE XUVF02(SHIFT,M,N,D,E,C,S,WANTLT,CL,SL,WANTRT,CR,SR)
C     MARK 13 RELEASE. NAG COPYRIGHT 1988.
C
C  XUVF02 performs a QR step on a bidiagonal matrix A.
C
...
C  is determined by XUVF02 so that
C
C     ( d( m )  0 ) := ( d( m )  e( m ) )*( c  -s )
C                                         ( s   c )
C
...
C
C  Thus XUVF02 performs the transformation
C
C     A := Q*A*P',
C
...
C     .. Scalar Arguments ..
      ...
      CHARACTER*1       SHIFT
C     .. Array Arguments ..
      REAL              CL(*), CR(*), D(*), E(*), SL(*), SR(*)
C     .. Local Scalars ..
      REAL              CO, CS, EI, SI, SN, TEMP
      INTEGER           I
