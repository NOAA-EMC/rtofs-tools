...
5.  **`convert_analinc_nc.py`**: Python engine that reads unformatted NCODA Fortran binary files (`_analinc`), parses metadata (dimensions, variable name) from the filename, attaches lat/lon grids, and exports to CF-compliant NetCDF.
6.  **`analinc_to_nc.sh`**: Bash wrapper for the NCODA increment conversion tool. Ensures output directories exist and validates the output NetCDF file.
...
- Output:
  - The script generates a PNG file in your current working directory named using the archive's base name and the extracted variables, for example: `rtofs_glo.t00z.n-24.archv_temp_salin.png`.

---

## 3. Converting NCODA Analysis Increments to NetCDF

Use the `analinc_to_nc.sh` wrapper to convert unformatted Fortran binary `_analinc` files generated by the NCODA data assimilation system into CF-compliant NetCDF files.

`./analinc_to_nc.sh <path_to_analinc_file> <output_directory>`

## Notes:
- `path_to_analinc_file`: Full path to the NCODA `_analinc` file (e.g., `seatmp_lyr_1o4500x3298_2026080200_0000_analinc`). The script automatically parses the variable name, grid dimensions, and layer depth from the filename and file size.
- `output_directory`: Directory to save the resulting `.nc` file. The wrapper script will automatically create this directory if it does not exist.

## Examples:
- Convert a 3D salinity increment file and save it to a custom directory:
  `./analinc_to_nc.sh /lfs/h1/ops/prod/com/rtofs/v2.5/rtofs.20260803/ncoda/hycom_var/restart/salint_lyr_1o4500x3298_2026080200_0000_analinc /lfs/h2/emc/ptmp/santha.akella/xx/`
- Output:
  - Generates a CF-compliant NetCDF file named identically to the input with a `.nc` extension (e.g., `salint_lyr_1o4500x3298_2026080200_0000_analinc.nc`) containing mapped `latitude`, `longitude`, and `layer` coordinates.
