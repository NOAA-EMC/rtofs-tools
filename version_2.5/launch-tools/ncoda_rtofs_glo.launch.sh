...
#if /lfs/h2/emc/ptmp is full
#export tmproot=/lfs/h2/emc/couple/noscrub/$LOGNAME/ptmp/$simulation
...
export GESROOT=${COMtmp}/$envir/com/$NET/$ver/nwges
export GESOUT=$GESROOT/${RUN}.${today}
export GESIN=$GESROOT/${RUN}.${today}
...
if [ $configname ==  release.v2.4.0.A.config ]; then
testjustthisjob=0
analysisanditspost=0
forecast1anditspost=0
fi
...
if [[ $rungzip -eq 1 && $fcstdays_step1 -eq 4 ]]
then
...
#!/bin/bash
#PBS -N $simulation.RTOFS_NCODA_QC
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_GLBL_VAR
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_POLAR_VAR
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_HYCOM_VAR
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_NCODA_INC
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_PRE_ANAL
#PBS -j oe
#PBS -A $account
...
export jobid=jrtofs_fcst1_pre
export job=$jobid
mkdir -p ${myDATAROOT}/$jobid

cat << EOF_fcst1_pre > $batchloc/rtofs.fcst1_pre.$pid
#!/bin/bash
#PBS -N $simulation.RTOFS_PRE_FCST1
#PBS -j oe
#PBS -A $account
#PBS -l place=vscatter,select=1:ncpus=1:mem=10GB
#PBS -q dev
...
export jobid=jrtofs_forecast_post
...
#############
#Submit forecast step1 gzip (issue - gzip for forecasts only work with 4-day forecasts)  (FIXED?)
#if [[ $rungzip -eq 1 && $fcstdays_step1 -eq 4 ]]
#then
export jobid=jrtofs_gzip_06
export job=$jobid
...
do
export jobid=jrtofs_global_gempak_${reg}
export job=$jobid
mkdir -p ${myDATAROOT}/$jobid
cat << EOF_gempak > $batchloc/rtofs.gempak.$reg.$pid
#!/bin/bash
#PBS -N $simulation.RTOFS_GEMPAK_${reg}
#PBS -j oe
#PBS -A RTOFS-DEV
#PBS -l place=vscatter,select=1:ncpus=24:mem=4GB
#PBS -q dev
...
export jobid=jrtofs_fcst2_pre
export job=$jobid
mkdir -p ${myDATAROOT}/$jobid

cat << EOF_fcst2_pre > $batchloc/rtofs.fcst2_pre.$pid
#!/bin/bash
#PBS -N $simulation.RTOFS_PRE_FCST2
#PBS -j oe
#PBS -A $account
...
#!/bin/bash
#PBS -N $simulation.RTOFS_FORECAST_STEP2
#PBS -j oe
#PBS -A $account
#PBS -l place=vscatter:exclhost,select=15:ncpus=120:mpiprocs=120
#PBS -q dev
...
if [[ $rungzip -eq 1 && $fcstdays_step2 -eq 4 ]]
then
export jobid=jrtofs_gzip_12
export job=$jobid
mkdir -p ${myDATAROOT}/$jobid
cat << EOF_gzip12 > $batchloc/rtofs.gzip12.$pid
#!/bin/bash
#PBS -N $simulation.RTOFS_GZIP12
#PBS -j oe
#PBS -A RTOFS-DEV
#PBS -l place=vscatter,select=1:ncpus=22:mem=22GB
#PBS -q dev
...
#!/bin/bash
#PBS -N $simulation.RTOFS_FCST_POST2
#PBS -j oe
#PBS -A RTOFS-DEV
...
#!/bin/bash
#PBS -N $simulation.RTOFS_FORECAST_STEP2_GRIB_POST_D$NN
#PBS -j oe
#PBS -A $account
#PBS -l place=vscatter,select=1:ncpus=11:mem=16GB
#PBS -q dev
...
#!/bin/bash
#PBS -N $simulation.RTOFS_FORECAST_STEP2_POST_D$NN
#PBS -j oe
#PBS -A $account
#PBS -l place=vscatter,select=1:ncpus=4:mem=120GB
#PBS -q dev
...
#!/bin/bash
#PBS -N $simulation.RTOFS_CHECK_OUTPUTS
#PBS -j oe
#PBS -A $account
