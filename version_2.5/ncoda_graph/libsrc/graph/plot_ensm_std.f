      subroutine plot_ensm_std (dir_path, date, dtg, nest, m, n, l, 
     *                          depth, lvl, igrid, rlat, stdlt1, 
...
c      @(#)$Id$
c
c MODULE NAME:  plot_ensm_std
c
c DESCRIPTION:  maps ncoda_et ensmeble standard deviation fields
c      
c PARAMETERS:
...
c
      do nv = 1, n_var
c
c        ..check for and read std field
c
         write (sfx, '(a, ''_'', a, ''_mem'', i3.3, ''_nest'', i2.2,
     *                 ''.'', a)') fld_name(nv), typ, zero, nest, dtg
...
c
            do i = 1, (m * n)
               if (depth(i) .lt. (lvl(z_plot(k))+1.01)) then
                  btm_msk(i) = -1.
               else
                  btm_msk(i) = 1.
               endif
...
c           ..set plot title
c
            z_lev = int (lvl(z_plot(k)) + .001)
            write (title, '(a, 3x, ''STD'', 3x, i4, '' M Depth'')')
     *                      fld_lbl(nv), z_lev
c
c           ..plot field
c
