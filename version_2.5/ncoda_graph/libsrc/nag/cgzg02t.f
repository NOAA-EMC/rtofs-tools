      SUBROUTINE CGZG02(NTOT,NVARS,NIND,SSP,ISSP,R,IR,RESULT,COEFF,
...
C
C     NAG AUXILIARY ROUTINE CGZG02
C     WRITTEN  7.10.73 BY PAUL GRIFFITHS (OXFORD UNIVERSITY)
C
C     DOES ALL THE WORK FOR NAG LIBRARY ROUTINES G02CGE-G02CHE.
C
C     USES NAG LIBRARY ROUTINES X02AKE, X02ALE
C
C     INPUT:
...
C     .. Array Arguments ..
      REAL              C(IC,NIND), COEFF(ICOEFF,3), R(IR,NVARS),
     *                  RESULT(13), RINV(IRINV,NIND), SSP(ISSP,NVARS)
C     .. Local Scalars ..
      REAL              A, B, BIGNUM, D, FIND, FTOT, SMALL
      INTEGER           I, J
C     .. External Functions ..
      REAL              X02AKE, X02ALE
      EXTERNAL          X02AKE, X02ALE
...
C
  ...
  100 CONTINUE
      ...
      RESULT(13) = 0.0
      D = RESULT(7)
      IF (D.LE.0.0) GO TO 140
      RESULT(10) = SQRT(D)
      IF (D.LE.1.0) GO TO 120
      RESULT(4) = 0.0
      IF (ABS(RESULT(3)).GE.D*SMALL) RESULT(4) = RESULT(3)/D
      GO TO 140
  120 RESULT(4) = BIGNUM
      IF (ABS(RESULT(3)).LT.D*BIGNUM) RESULT(4) = RESULT(3)/D
  140 IF (A.LE.0.0) GO TO 160
      RESULT(13) = 1.0 - D*FTOT/A
      A = B/A
      IF (A.GT.0.0) RESULT(11) = SQRT(A)
      RESULT(12) = A
C
...
C
  160 A = RESULT(10)
      DO 200 I = 1, NIND
         B = 0.0
         IF (C(I,I).GT.0.0) B = A*SQRT(C(I,I))
         COEFF(I,2) = B
         IF (B.LE.1.0) GO TO 180
         COEFF(I,3) = 0.0
