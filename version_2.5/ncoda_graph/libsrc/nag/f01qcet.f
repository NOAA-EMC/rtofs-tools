      SUBROUTINE F01QCE(M,N,A,LDA,ZETA,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C
C  1. Purpose
C     =======
C
C  F01QCE  finds  the  QR factorization  of the real  m by n,  m .ge. n,
...
C  NAG Library routine  F01QDE. The  operation  B := Q*B can be obtained
C  by the call:
C
C     IFAIL = 0
C     CALL F01QDE( 'No transpose', 'Separate', M, N, A, LDA, ZETA,
C    $             K, B, LDB, WORK, IFAIL )
C
C  and  B := Q'*B  can be obtained by the call:
C
C     IFAIL = 0
C     CALL F01QDE( 'Transpose', 'Separate', M, N, A, LDA, ZETA,
C    $             K, B, LDB, WORK, IFAIL )
C
...
C  parameter  LDB  can be replaced by  M. See routine F01QDE for further
C  details.
C
...
C  F01QEE, which overwrites the k columns of Q on the first k columns of
C  the array A.  Q is obtained by the call:
C
C     CALL F01QEE( 'Separate', M, N, K, A, LDA, ZETA, WORK, IFAIL )
C
...
C  F02WUE  can be  used to  determine  the  singular value decomposition
C  of  R.
C
C
...
C     .. External Subroutines ..
      EXTERNAL          SGEMV, SGER, F06FRE, ABYP01
...
C
      IERR = 0
      IF (M.LT.N) CALL ABYP01(M,'M',IFAIL,IERR,SRNAME)
...
C
         CALL F06FRE(M-K,A(K,K),A(K+1,K),1,ZERO,ZETA(K))
...
C           ZETA( k + 1 ), ..., ZETA( n ). )
C
            CALL SGEMV('Transpose',M-K+1,N-K,ONE,A(K,K+1),LA,A(K,K),1,
     *                 ZERO,ZETA(K+1),1)
C
C           Now form  B := B - u*work'.
C
            CALL SGER(M-K+1,N-K,-ONE,A(K,K),1,ZETA(K+1),1,A(K,K+1),LA)
C
C           Restore beta.
C
            A(K,K) = TEMP
