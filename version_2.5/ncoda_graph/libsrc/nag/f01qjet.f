      SUBROUTINE F01QJE(M,N,A,LDA,ZETA,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C
C  1. Purpose
C     =======
C
C  F01QJE  finds the  RQ factorization  of the  real  m by n,  m .le. n,
...
C  by calling NAG Library routine F01QKE, which overwrites the k rows of
C  P'  on the first  k rows of the array A.  P' is obtained by the call:
C
C     IFAIL = 0
C     CALL F01QKE( 'Separate', M, N, K, A, LDA, ZETA, WORK, IFAIL )
C
...
C     .. External Subroutines ..
      EXTERNAL          SGEMV, SGER, WEXF02, ABYP01
...
C
         BETA = A(K,K)
         CALL WEXF02(K-1,N-M,BETA,A(K,1),LDA,A(K,MP1),LDA,ZERO,ZETA(K))
...
C
            CALL SGEMV('No transpose',K-1,K,ONE,A,LDA,A(K,1),LDA,ZERO,
     *                 ZETA,1)
C
...
C           and       A2 := A2 - v*z( k )'.
C
            CALL SGER(K-1,K,-ONE,ZETA,1,A(K,1),LDA,A,LDA)
            CALL SGER(K-1,N-M,-ONE,ZETA,1,A(K,MP1),LDA,A(1,MP1),LDA)
         END IF
         A(K,K) = BETA
   20 CONTINUE
C
