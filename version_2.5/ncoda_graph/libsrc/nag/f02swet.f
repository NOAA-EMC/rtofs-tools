      SUBROUTINE F02SWE(N,A,LDA,D,E,NCOLY,Y,LDY,WANTQ,Q,LDQ,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C
C  1. Purpose
C     =======
C
C  F02SWE  reduces the  n by n upper triangular matrix  R  to bidiagonal
...
C  c and s  may be recovered from  t  by a call to routine  F06BCE.  See
...
C  obtained by calls to the auxiliary linear algebra routine F02SXE. The
C  matrix  W = P'*X,  where  X is an  nrowx by n matrix, may be found by
C  the call
C
C     IFAIL = 0
C     CALL F02SXE( N, A, LDA, NROWX, X, LDX, WORK, IFAIL )
C
...
C
C     IFAIL = 0
C     CALL F02SXE( N, A, LDA, 0, DUMMY, 1, WORK, IFAIL )
C
C  where  A must be as returned from  F02SWE.  P' will be overwritten on
...
C  routine  F02SXE  for further details.
C
C
...
C     .. Parameters ..
      ...
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F02SWE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDA, LDQ, LDY, N, NCOLY
      LOGICAL           WANTQ
C     .. Array Arguments ..
      REAL              A(LDA,*), D(*), E(*), Q(LDQ,*), Y(LDY,*)
C     .. Local Scalars ..
      INTEGER           I, IERR, J, K
C     .. Local Arrays ..
      CHARACTER*80      REC(1)
...
C     .. External Subroutines ..
      EXTERNAL          SCOPY, F06FQE, F06QTE, F06QXE, ABYP01
...
C
      ...
      IF (LDA.LT.N) CALL ABYP01(LDA,'LDA',IFAIL,IERR,SRNAME)
      IF (NCOLY.LT.0) CALL ABYP01(NCOLY,'NCOLY',IFAIL,IERR,SRNAME)
      IF ((NCOLY.GT.0) .AND. (LDY.LT.N)) CALL ABYP01(LDY,'LDY',IFAIL,
     *    IERR,SRNAME)
      IF ((WANTQ) .AND. (LDQ.LT.N)) CALL ABYP01(LDQ,'LDQ',IFAIL,IERR,
     *    SRNAME)
      IF (IERR.GT.0) THEN
...
C        d.
C
         CALL F06FQE('Variable pivot','Backwards',N-K-1,A(K,K+1),
     *               A(K,K+2),LDA,E(K+1),D(K+1))
C
C        Form  R( k ) = Q( k )*R( k - 1 )*P( k )'. The cosines and sines
C        that define Q( k ) are overwritten on e and d.
C
         CALL F06QTE('Right side',N-K,1,N-K,E(K+1),D(K+1),A(K+1,K+1),
     *               LDA)
C
C        Form Q( k )*Y.
C
         IF (NCOLY.GT.0) CALL F06QXE('Left side','Variable pivot',
...
C
         IF (WANTQ) THEN
            CALL SCOPY(N-K-1,E(K+1),1,Q(K,K+2),LDQ)
            CALL SCOPY(N-K-1,D(K+1),1,Q(K+2,K),1)
         END IF
   20 CONTINUE
      IF (WANTQ) THEN
C
...
C
   ...
   40             CONTINUE
                  ...
                  DO 60 I = K + 2, N
                     ...
                     Q(I,K) = ZERO
   60             CONTINUE
                  CALL F06QXE('Left side','Variable pivot','Forward',
