      SUBROUTINE F02SYE(N,D,E,NCOLB,B,LDB,NROWY,Y,LDY,NCOLZ,Z,LDZ,WORK,
     *                  IFAIL)
...
C     =======
C
C  F02SYE  reduces  an  n by n  real bidiagonal matrix  to diagonal form
...
C
C           E  is  used  as  internal  workspace  by  F02SYE  and so the
...
C
C           The array  WORK  is used as  internal workspace  by  F02SYE.
...
C     ===================
C
C  This routine  can be  used in conjunction  with routines  F02SWE  and
C  F02SXE   to  find  the  singular  value  decomposition  of  an  upper
C  triangular matrix.
C
C
...
C     .. Parameters ..
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='F02SYE')
      REAL              QTR, ZERO
      PARAMETER         (QTR=0.25E+0,ZERO=0.0E+0)
C     .. Scalar Arguments ..
      INTEGER           IFAIL, LDB, LDY, LDZ, N, NCOLB, NCOLZ, NROWY
C     .. Array Arguments ..
      REAL              B(LDB,*), D(*), E(*), WORK(*), Y(LDY,*),
     *                  Z(LDZ,*)
C     .. Local Scalars ..
      REAL              AMAX, CS, DMAX, EKM2, SN, TEMP
...
C     .. External Subroutines ..
      EXTERNAL          SSCAL, XUSF02, XUTF02, XUUF02, XUVF02, XUWF02,
     *                  F06FGE, F06QKE, F06QXE, ABYP01
...
C
      ...
      WANTZ = NCOLZ .GT. 0
      IERR = 0
      IF (N.LT.0) CALL ABYP01(N,'N',IFAIL,IERR,SRNAME)
      IF (NCOLB.LT.0) CALL ABYP01(NCOLB,'NCOLB',IFAIL,IERR,SRNAME)
      IF ((WANTB) .AND. (LDB.LT.N)) CALL ABYP01(LDB,'LDB',IFAIL,IERR,
     *    SRNAME)
      IF (NROWY.LT.0) CALL ABYP01(NROWY,'NROWY',IFAIL,IERR,SRNAME)
      IF ((WANTY) .AND. (LDY.LT.NROWY)) CALL ABYP01(LDY,'LDY',IFAIL,
     *    IERR,SRNAME)
      IF (NCOLZ.LT.0) CALL ABYP01(NCOLZ,'NCOLZ',IFAIL,IERR,SRNAME)
      IF ((WANTZ) .AND. (LDZ.LT.N)) CALL ABYP01(LDZ,'LDZ',IFAIL,IERR,
     *    SRNAME)
      IF (IERR.GT.0) THEN
         WRITE (REC,FMT=99999) IERR
         IFAIL = P01ABE(IFAIL,-1,SRNAME,1,REC)
         RETURN
      END IF
      IF (N.EQ.0) THEN
         IFAIL = P01ABE(IFAIL,0,SRNAME,0,REC)
         RETURN
      END IF
C
...
C
      IF (AMAX.GT.ZERO) THEN
         CALL SSCAL(N,1/AMAX,D,1)
         CALL SSCAL(N-1,1/AMAX,E,1)
      END IF
C
...
C
               CALL XUSF02(K,D,E,WANTZ,WORK(IW2),WORK(IW3))
               IF (WANTZ) CALL F06QXE('Left','Bottom','Backward',N,
...
C              Force a split.
C
               CALL XUWF02(P,K,D,E,(WANTB) .OR. (WANTY),WORK,WORK(IW1))
               IF (WANTB) CALL F06QXE('Left','Top','Forward',N,NCOLB,P,
     *                                K,WORK,WORK(IW1),B,LDB)
               IF (WANTY) CALL F06QXE('Right','Top','Forward',NROWY,N,P,
...
C
            ...
            ELSE
               EKM2 = ZERO
            END IF
            CALL XUUF02(-1,D(L),E(L),D(K-1),D(K),EKM2,E(K-1),CS,SN)
            CALL XUVF02('Non-zero',L,K,D,E,CS,SN,(WANTB) .OR. (WANTY),
     *                  WORK,WORK(IW1),WANTZ,WORK(IW2),WORK(IW3))
            IF (WANTB) CALL F06QXE('Left','Variable','Forward',N,NCOLB,
     *                             L,K,WORK,WORK(IW1),B,LDB)
            IF (WANTY) CALL F06QXE('Right','Variable','Forward',NROWY,N,
     *                             L,K,WORK,WORK(IW1),Y,LDY)
            IF (WANTZ) CALL F06QXE('Left','Variable','Forward',N,NCOLZ,
...
C
      IF (AMAX.GT.ZERO) THEN
         CALL SSCAL(N,AMAX,D,1)
         CALL SSCAL(N-1,AMAX,E,1)
      END IF
C
...
C
      DO 60 I = K, N
         IF (D(I).LT.ZERO) THEN
            D(I) = -D(I)
            IF (WANTB) CALL F06FGE(NCOLB,B(I,1),LDB)
            IF (WANTY) CALL F06FGE(NROWY,Y(1,I),1)
         END IF
   60 CONTINUE
C
...
C
  ...
  100    CONTINUE
         ...
         END IF
  120 CONTINUE
      IF (WANTB) CALL F06QKE('Left','Transpose',N,WORK,NCOLB,B,LDB)
      IF (WANTY) CALL F06QKE('Right','No Transpose',N,WORK,NROWY,Y,LDY)
      IF (WANTZ) CALL F06QKE('Left','Transpose',N,WORK,NCOLZ,Z,LDZ)
      WORK(1) = ITER
      IF (K.EQ.1) THEN
         IFAIL = P01ABE(IFAIL,0,SRNAME,0,REC)
      ELSE
         WRITE (REC,FMT=99998) K
         IFAIL = P01ABE(IFAIL,K,SRNAME,2,REC)
      END IF
      RETURN
C
C
C     End of F02SYE. ( SBIQR )
C
