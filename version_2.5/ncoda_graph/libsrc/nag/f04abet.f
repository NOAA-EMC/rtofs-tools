      SUBROUTINE F04ABE(A,IA,B,IB,N,M,C,IC,WKSPCE,BB,IBB,IFAIL)
...
C     .. External Functions ..
      REAL              X02AJE
      INTEGER           P01ABE
      EXTERNAL          X02AJE, P01ABE
C     .. External Subroutines ..
      EXTERNAL          F04AFE, F07FDE
...
C                 Perform iterative refinement on the solution.
                  EPS = X02AJE()
                  IERR2 = 1
                  CALL F04AFE(N,M,A,IA,WKSPCE,B,IB,EPS,C,IC,BB,IBB,NIT,
     *                        IERR2)
                  IF (IERR2.EQ.1) THEN
                     IERR = 2
                     NREC = 2
...
C              Copy the diagonal elements of A back from WKSPCE.
  ...
  100          CONTINUE
      ...
      END IF
C
      IFAIL = P01ABE(IFAIL,IERR,SRNAME,NREC,P01REC)
      RETURN
C
