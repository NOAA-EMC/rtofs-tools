      SUBROUTINE F06FQE(PIVOT,DIRECT,N,ALPHA,X,INCX,C,S)
...
C     ..
C
C  F06FQE generates the parameters of an orthogonal matrix P such that
C
      ...
      IF (N.GT.0) THEN
         IF ((DIRECT.EQ.'B') .OR. (DIRECT.EQ.'b')) THEN
            ...
            IF ((PIVOT.EQ.'V') .OR. (PIVOT.EQ.'v')) THEN
               DO 10 I = N, 2, -1
                  CALL F06BAE(X(IX-INCX),X(IX),C(I),S(I))
                  IX = IX - INCX
   10          CONTINUE
               CALL F06BAE(ALPHA,X(IX),C(1),S(1))
...
C
               DO 20 I = N, 1, -1
                  CALL F06BAE(ALPHA,X(IX),C(I),S(I))
                  S(I) = -S(I)
                  X(IX) = -X(IX)
                  IX = IX - INCX
   20          CONTINUE
...
C
               DO 30 I = 1, N - 1
                  ...
                  IX = IX + INCX
   30          CONTINUE
               CALL F06BAE(ALPHA,X(IX),C(N),S(N))
               S(N) = -S(N)
               X(IX) = -X(IX)
            ELSE IF ((PIVOT.EQ.'F') .OR. (PIVOT.EQ.'f')) THEN
               DO 40 I = 1, N
