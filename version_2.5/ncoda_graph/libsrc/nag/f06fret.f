      SUBROUTINE F06FRE(N,ALPHA,X,INCX,TOL,ZETA)
...
C     ..
C
C  F06FRE generates details of a generalized Householder reflection such
C  that
C
...
C
            SSQ = ONE
            SCALE = ZERO
            CALL F06FJE(N,X,INCX,SCALE,SSQ)
C
...
C
            IF ((SCALE.EQ.ZERO) .OR. (SCALE.LE.MAX(EPS*ABS(ALPHA),TOL)))
     *          THEN
               ZETA = ZERO
            ELSE IF (ALPHA.EQ.ZERO) THEN
               ZETA = ONE
               ALPHA = SCALE*SQRT(SSQ)
               CALL SSCAL(N,-1/ALPHA,X,INCX)
            ELSE
               ...
               CALL SSCAL(N,-1/(ZETA*BETA),X,INCX)
               ALPHA = BETA
            END IF
         END IF
      END IF
