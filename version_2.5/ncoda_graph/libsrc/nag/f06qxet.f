      SUBROUTINE F06QXE(SIDE,PIVOT,DIRECT,M,N,K1,K2,C,S,A,LDA)
...
C     ..
C
C  F06QXE  performs the transformation
C
...
C     .. Executable Statements ..
      ...
      IF ((MIN(M,N,K1).LT.1) .OR. (K2.LE.K1) .OR. ((LEFT)
     *    .AND. (K2.GT.M)) .OR. ((RIGHT) .AND. (K2.GT.N))) RETURN
      IF (LEFT) THEN
