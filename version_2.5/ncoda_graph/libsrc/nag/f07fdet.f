      SUBROUTINE F07FDE(UPLO,N,A,LDA,INFO)
...
C     .. External Subroutines ..
      EXTERNAL          AAZF06, FDZF07, ZAZF07, SGEMM, SSYRK, STRSM
...
C
      ...
      END IF
      IF (INFO.NE.0) THEN
         CALL AAZF06('F07FDE/SPOTRF',-INFO)
         RETURN
      END IF
C
...
C
      CALL ZAZF07(1,'F07FDE',NB,0)
      IF (NB.LE.1) THEN
C
C        Use unblocked code.
C
         CALL FDZF07(UPLO,N,A,LDA,INFO)
      ELSE
C
C        Use blocked code.
C
...
C
     ...
     *                    ONE,A(J,J),LDA)
               CALL FDZF07('Upper',JB,A(J,J),LDA,INFO)
...
C
     ...
     *                    ONE,A(J,J),LDA)
               CALL FDZF07('Lower',JB,A(J,J),LDA,INFO)
               IF (INFO.NE.0) GO TO 60
               IF (J+JB.LE.N) THEN
C
