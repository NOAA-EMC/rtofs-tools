      SUBROUTINE G02CAE(N,X,Y,RESULT,IFAIL)
...
C     NAG LIBRARY ROUTINES    X02AKE, X02ALE
C
C
...
C     .. Parameters ..
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='G02CAE')
C     .. Scalar Arguments ..
      INTEGER           IFAIL, N
C     .. Array Arguments ..
      REAL              RESULT(20), X(N), Y(N)
C     .. Local Scalars ..
      REAL              BIGNUM, FN, R, SMALL, STDX, STDY, XBAR, XI,
     *                  YBAR, YI
      INTEGER           I, IERROR
C     .. Local Arrays ..
...
C     .. Executable Statements ..
   ...
   20 CONTINUE
      ...
      DO 40 I = 1, N
         ...
         R = R + XI*YI
   40 CONTINUE
      IF (STDX.LE.0.0 .OR. STDY.LE.0.0) GO TO 200
C
...
C
  ...
  100 YBAR = SQRT(YI/STDX)
      XBAR = SQRT(YI*(1.0/(FN+1.0)+XBAR*XBAR/STDX))
      RESULT(8) = YBAR
      RESULT(9) = XBAR
      IF (YBAR.LE.1.0) GO TO 120
      RESULT(10) = 0.0
...
C
  180 IFAIL = 0
      RETURN
  200 IERROR = 2
  220 IFAIL = P01ABE(IFAIL,IERROR,SRNAME,0,P01REC)
      RETURN
      END
