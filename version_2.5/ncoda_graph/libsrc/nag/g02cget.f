      SUBROUTINE G02CGE(NCASES,NVARS,NIND,XBAR,SSP,ISSP,R,IR,RESULT,
...
C     NAG LIBRARY ROUTINE    F04ABE
C     AND AUXILIARY ROUTINES CGZG02
C     CGYG02
C
C
...
C     .. Parameters ..
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='G02CGE')
C     .. Scalar Arguments ..
      INTEGER           IC, ICOEFF, IFAIL, IR, IRINV, ISSP, IWKZ,
     *                  NCASES, NIND, NVARS
C     .. Array Arguments ..
      REAL              C(IC,NIND), COEFF(ICOEFF,3), CONST(3),
...
C
      IERROR = 1
      CALL F04ABE(R,IR,C,IC,NIND,NIND,RINV,IRINV,COEFF,WKZ,IWKZ,IERROR)
C
C     RE-CONSTITUTE LOWER TRIANGLE OF ORIGINAL MATRIX WHICH HAS
C     BEEN ALTERED BY F04ABE
C
      DO 100 J = 1, NIND
         DO 80 I = J, NIND
            R(I,J) = R(J,I)
  ...
  100 CONTINUE
C
C     BRANCH IF ERROR OCCURRED IN F04ABE
C
      IF (IERROR.NE.0) GO TO 120
C
C     GET CGZG02 TO DO ALL THE HARD WORK
C
      NTOT = NCASES - 1
      CALL CGZG02(NTOT,NVARS,NIND,SSP,ISSP,R,IR,RESULT,COEFF,ICOEFF,
     *            RINV,IRINV,C,IC)
C
C     NOW CALCULATE THREE ITEMS RELATING TO THE REGRESSION CONSTANT
C     USING CGYG02
C
      CALL CGYG02(NCASES,NVARS,NIND,XBAR,RESULT(10),COEFF,ICOEFF,CONST,
     *            C,IC)
C
...
C
      ...
      RETURN
  120 IERROR = IERROR + 4
  140 IFAIL = P01ABE(IFAIL,IERROR,SRNAME,0,P01REC)
      RETURN
      END
