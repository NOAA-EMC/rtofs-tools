      SUBROUTINE S14BAE(A,X,TOL,P,Q,IFAIL)
C     MARK 14 RELEASE. NAG COPYRIGHT 1989.
C     .. Parameters ..
      REAL              ZERO, ONE, HALF, QUARTR, THRTWO, TWO, THREE,
     *                  FOUR, EIGHT, TWENTY, PSEVEN, ONEP4
      PARAMETER         (ZERO=0.0E0,ONE=1.0E0,HALF=0.5E0,QUARTR=0.25E0,
     *                  THRTWO=1.5E0,TWO=2.0E0,THREE=3.0E0,FOUR=4.0E0,
     *                  EIGHT=8.0E0,TWENTY=20.0E0,PSEVEN=0.7E0,
     *                  ONEP4=1.4E0)
      INTEGER           MAXIT, NTERMS
      PARAMETER         (MAXIT=600,NTERMS=22)
      CHARACTER*6       SRNAME
      PARAMETER         (SRNAME='S14BAE')
C     .. Scalar Arguments ..
      REAL              A, P, Q, TOL, X
      INTEGER           IFAIL
C     .. Local Scalars ..
      REAL              ALG, ALGP1, ALGS, ALPHA, ALX, AP1, BOT, EPS,
...
C     .. External Functions ..
      REAL              S14AAE, S14ABE, BAZS14, X02AJE, X02AME
      INTEGER           P01ABE
      EXTERNAL          S14AAE, S14ABE, BAZS14, X02AJE, X02AME, P01ABE
C     .. Intrinsic Functions ..
      INTRINSIC         ABS, EXP, LOG, MAX, SQRT
C     .. Data statements ..
      DATA              C/5.7721566490153286060651209008E-1,
     *                  -6.5587807152025388107701951515E-1,
     *                  -4.200263503409523552900393488E-2,
     *                  1.665386113822914895017007951E-1,
     *                  -4.21977345555443367482083013E-2,
     *                  -9.6219715278769735621149217E-3,
     *                  7.21894324666309954230950103E-3,
     *                  -1.16516759185906511211971E-3,
     ...
     *                  -2.056338416977607103E-7, 6.1160951044814158E-9,
     *                  5.0020076444692229E-9, -1.181274570487020E-9,
     *                  1.04342671169110E-10, 7.782263439905E-12,
     *                  -3.696805618642E-12, 5.1003702875E-13,
     *                  -2.058326054E-14/
...
C           Use the asymptotic expansion of Temme.
            UNDFL = LOG(SAFE)
            IF (A.LE.X) THEN
               Q = BAZS14(A,X,.FALSE.,EPS,UNDFL)
               P = ONE - Q
            ELSE
               P = BAZS14(A,X,.TRUE.,EPS,UNDFL)
               Q = ONE - P
            END IF
         ELSE
            ALX = LOG(X)
...
C                 decimal place accuracy in the summation below.
  ...
  140 IFAIL = P01ABE(IFAIL,IERR,SRNAME,NREC,REC)
      RETURN
C
99999 FORMAT (1X,'** On entry, A.le.0.0 : A =',1P,E13.5)
99998 FORMAT (1X,'** On entry, X.lt.0.0 : X =',1P,E13.5)
